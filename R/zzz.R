...
.onAttach <- function(libname, pkgname) {
  with(canprot, {
    ...
    human_base <- readRDS(system.file("/extdata/aa/human_base.rds", package = "canprot"))
    #packageStartupMessage(paste("human_base:", nrow(human_base), "proteins"))

    human_additional <- readRDS(system.file("/extdata/aa/human_additional.rds", package = "canprot"))
    #packageStartupMessage(paste("human_additional:", nrow(human_additional), "proteins"))

    human_extra <- read.csv(system.file("/extdata/aa/human_extra.csv", package = "canprot"), as.is = TRUE)
    ...
    uniprot_updates <- read.csv(system.file("/extdata/aa/uniprot_updates.csv", package = "canprot"), as.is = TRUE)
  })
}
