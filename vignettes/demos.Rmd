...
- `calc.metrics()`: Calculates metrics named in an argument. The metrics calculated here are:
...
- `add.hull()`: Adds a convex hull around data points
...
- `read.fasta()`: Reads a FASTA sequence file and returns amino acid compositions of proteins. Additional processing is performed by using the following arguments:
